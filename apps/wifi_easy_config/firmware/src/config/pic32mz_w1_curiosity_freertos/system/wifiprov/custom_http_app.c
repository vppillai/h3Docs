...
//    TCPIP_MAC_ADDR newMACAddr;
    ...
    // Read all browser POST data
     ...
     if(!strncmp((char *)httpDataBuff+Index, (const char *)"saveconfig=",strlen("saveconfig=")))
     {
        ...
        wificonfig.save_config = CONVERT_TO_NUMBER(httpDataBuff[Index]);
     ...
     if(!strncmp((char *)httpDataBuff+Index, (const char *)"countrycode=",strlen("countrycode=")))
     {
        ...
        switch(country_code)
        {
            ...
            case 2: //EMEA
                ...
                break;
            case 3: //JPN
                strcpy((char *)wificonfig.countrycode,(const char *)"JPN");
                break;    
        }       
    ...
    if(!strncmp((char *)httpDataBuff+Index, (const char *)"staauth=",strlen("staauth=")))
     {
        ...
        wificonfig.staconfig.auth_type = CONVERT_TO_NUMBER(httpDataBuff[Index]);
    ...
    if(!strncmp((char *)httpDataBuff+Index, (const char *)"staauto=",strlen("staauto=")))
     {
        ...
        wificonfig.staconfig.auto_connect = CONVERT_TO_NUMBER(httpDataBuff[Index]);
     }
     Index += 2;
    ...
    if(!strncmp((char *)httpDataBuff+Index, (const char *)"apauth=",strlen("apauth=")))
     {
        ...
        Index += strlen("apauth=");
        wificonfig.apconfig.auth_type = CONVERT_TO_NUMBER(httpDataBuff[Index]);
    ...
    if(!strncmp((char *)httpDataBuff+Index, (const char *)"ssidv=",strlen("ssidv=")))
     {
        ...
        Index += strlen("ssidv=");
        wificonfig.apconfig.ssid_visibility = CONVERT_TO_NUMBER(httpDataBuff[Index]);
...
{    
    //static bool call_register = false;
    //if(call_register == false)
    //{
        //SYS_WIFI_CtrlMsg(sysObj.syswifi, SYS_WIFI_REGCALLBACK, WiFiServCallback_HTTP, sizeof (uint8_t *));
        //SYS_CONSOLE_PRINT("%s:%d register callback\n",__FUNCTION__,__LINE__);
        //call_register =true;
    //}
    SYS_WIFI_CtrlMsg(sysObj.syswifi, SYS_WIFI_GETCONFIG, &wificonfig, sizeof(SYS_WIFI_CONFIG));
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    if(val == wificonfig.save_config)
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"SELECTED");
    //TCPIP_TCP_Put(sktHTTP, CONVERT_TO_ASCII(wificonfig.save_config));
}
void TCPIP_HTTP_Print_config_countrycode(HTTP_CONN_HANDLE connHandle,uint16_t val)
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    
   // TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)wificonfig.countrycode);
    switch(val)
    {
        case 0: //GEN
        ...
        case 2:  //EMEA
            ...
            break;
        case 3:  //JPN
            if(!strcmp((const char *)wificonfig.countrycode,"JPN"))
            {
                TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"SELECTED");            
            }
            break;
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    //TCPIP_TCP_Put(sktHTTP,CONVERT_TO_ASCII(wificonfig.staconfig.auto_connect));
    if(val == wificonfig.staconfig.auto_connect)
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"SELECTED");
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    //TCPIP_TCP_Put(sktHTTP,CONVERT_TO_ASCII(wificonfig.staconfig.auth_type));
    if(val == wificonfig.staconfig.auth_type)
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"SELECTED");
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    //TCPIP_TCP_Put(sktHTTP,CONVERT_TO_ASCII(wificonfig.apconfig.auth_type));
    if(val == wificonfig.apconfig.auth_type)
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"SELECTED");
}
...
{
    TCP_SOCKET sktHTTP = TCPIP_HTTP_CurrentConnectionSocketGet(connHandle);
    //TCPIP_TCP_Put(sktHTTP, CONVERT_TO_ASCII(wificonfig.apconfig.ssid_visibility));
    if(val == wificonfig.apconfig.ssid_visibility)
        TCPIP_TCP_StringPut(sktHTTP, (const uint8_t *)"SELECTED");
}
