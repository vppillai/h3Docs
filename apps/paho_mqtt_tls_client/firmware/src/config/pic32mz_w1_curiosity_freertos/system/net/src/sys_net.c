...
static bool SYS_NET_Ll_Link_Status(SYS_NET_Handle *hdl)
{
	...
	/* We Do not Check Lower Layer Status in case of TCP since KeepAlive takes care of it */
	...
	switch(hdl->status)
	{
        ...
        case SYS_NET_STATUS_WAIT_FOR_SNTP:
        {
            ...
            if(NET_PRES_SocketEncryptSocket(hdl->socket) != true) {
                SYS_NETDEBUG_ERR_PRINT(g_NetAppDbgHdl, NET_CFG, "NET_SRVC (%d): Negotiation Failed; Aborting\r\n");		
...
static void SYS_NET_Server_Task(SYS_NET_Handle *hdl)
{
	...
	switch(hdl->status)
	{
        ...
        case SYS_NET_STATUS_WAIT_FOR_SNTP:
        {
            ...
            if(NET_PRES_SocketEncryptSocket(hdl->socket) != true) {
                SYS_NETDEBUG_ERR_PRINT(g_NetAppDbgHdl, NET_CFG, "Negotiation Failed; Aborting\r\n");		
                hdl->status = SYS_NET_STATUS_TLS_NEGOTIATION_FAILED;
                break;
            }
            hdl->status = SYS_NET_STATUS_TLS_NEGOTIATING;
