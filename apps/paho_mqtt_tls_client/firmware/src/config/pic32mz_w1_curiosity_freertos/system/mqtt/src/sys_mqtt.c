...
static void SysMqtt_Command_Process(int argc,char *argv[])
{
    ...
    if((argc >= 2) && (!strcmp((char*)argv[1],"open")))
    {
        ...
        else
		{
			...
			int inst = inst = strtoul( argv[2], 0, 10);
...
*/
int32_t SYS_MQTT_TakeSemaphore(SYS_MQTT_Handle *hdl)
{
...
*/
void SYS_MQTT_GiveSemaphore(SYS_MQTT_Handle *hdl)
{
    OSAL_SEM_Post(&hdl->InstSemaphore);
}

bool SYS_MQTT_Ll_Status(SYS_MQTT_Handle *hdl)
{
    if(SYS_NET_GetStatus(hdl->netSrvcHdl) == SYS_NET_STATUS_CONNECTED)
        return true;
    return false;     
}
